...
//    ssc.sparkContext.addFile("file://"+this.getClass.getResource("/ip2region.db").getPath) //广播文件
    //打包到集群上面跑必须用下面这一行代码，
    // 因为Linux上没有"file://"+this.getClass.getResource("/ip2region.db").getPath这个目录；
    // 用IDEA跑上下的代码都行。
    ...
    ipDStream.foreachRDD(rdd => {
      ...
      try {
        ...
        val top3Rdd = resultRdd.sortBy[Long](_._2, false).take(3) //false表示倒序
  ...
  def calcPageJumpCount(sqlProxy: SqlProxy, item: (String, Int), client: Connection): Unit = {
    ...
    //对num 进行修改 并且判断当前page_id是否为首页（商品课程页）
    if (page_id == 1) {
