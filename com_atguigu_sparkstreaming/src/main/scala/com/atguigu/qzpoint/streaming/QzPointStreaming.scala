...
//      .setMaster("local[*]")
  ...
  def qzQuestionUpdate(key: String, iters: Iterable[(String, String, String, String, String, String)], sqlProxy: SqlProxy, client: Connection) = {
    ...
    //获取后边统计指标需要的参数，比如用户做题总个数，用户做过的questionid字符串，做正确题的个数
    val countSize = resultQuestionid.length   //用户做的知识点个数
    ...
    qz_sum += qzSum_history  //当前批次做题总数+做题总数历史值=做题总数（不去重）
    qz_istrue += istrue_history  //单签批次的做题正确数+正确数历史值=做题正确总个数
